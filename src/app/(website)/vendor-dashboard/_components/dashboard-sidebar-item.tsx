...
const DashboardSidebarItem = ({ item }: Props) => {
  ...
  return (
    <div className="relative pl-[38px]">
      <Link className={cn(`flex items-center gap-2 pl-[32px] py-[24px] rounded-l-full text-lg font-medium leading-[21px] text-[#121D42] dark:!text-[#6841A5] ${isActive && "bg-[#E6EEF6] dark:bg-[#482D721A] "}`)} href={item.href}>{item?.icon} {item?.name}</Link>
